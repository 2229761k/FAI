...
def main():
    ...
    trainval_loader = torch.utils.data.DataLoader(custom_loader,
                                                  ...
                                                  drop_last=True)
    option_val = option
    step=option_val.save_step
    option_val.data_split = 'test'
    custom_loader_val = data_loader.WholeDataLoader(option_val)
    trainval_loader_val = torch.utils.data.DataLoader(custom_loader_val,
                                                  batch_size=option.batch_size,
                                                  shuffle=True,
                                                  num_workers=option.num_workers,
                                                  drop_last=True)
    ...
    if option.is_train:
        ...
        # configure("tensorboard/logger_sigmoid")
        # configure("tensorboard/train11")
        # configure("tensorboard/train_same")
        # configure("tensorboard/train_reverse")
        configure("tensorboard/train_wo_reverse")
        # configure("tensorboard/train_wo_oneone")
        # configure("tensorboard/train_wo_same")
        save_option(option)
        trainer.train(trainval_loader, trainval_loader_val)
        # trainer.train(trainval_loader)
    else:
        trainer._validate(step, trainval_loader_val)
